...
class Program
{
    // https://tr.wikibooks.org/wiki/C_Sharp_Programlama_Dili/Temel_string_i%C5%9Flemleri
    public static void Main(string[] args)
    {
        // Tek Satırda İstediğimiz Kadar Yazı veya Karakter Yazmak

        // Aşağıdaki kodda ben 50 defa yan yana çizgi yazdırmak için gerekli kodları yazdım istediğinize göre siz değiştirebilirsiniz.
        ...
        Console.WriteLine(adSoyad.TrimStart()); // TrimStart'da ise string ifadenin başındaki boşluğu kaldırır sonlarındaki boşlukları kaldırmaz. Çıktı olarak "Atakan Yasin Uludağ  " yazacaktır. (Sonunda boşluk olacak şekilde)

        // Üçüncü Kullanım (TrimEnd)
        string adSoyad3 = "  Cihan Özhan     ";
        Console.WriteLine(adSoyad.TrimEnd()); // TrimEnd'da string ifadenin sonundaki boşluğu kaldırır başındaki boşlukları kaldırmaz. Çıktı olarak "  Atakan Yasin Uludağ" yazacaktır. (Başında boşluk olacak şekilde)
        ...
        /*
            ...
            This function is used to insert string inside another string, starting from the specified index. Have a look at the example.
         */
        ...
        // Join function takes a delimiting character and array of strings as an input and then join the words to form a string. Have a look at the following example.
